...
Task 7 默认关闭识图，不影响现有 DeepSeek 文字回复。真实视觉服务可能收费；程序每天最多尝试 5 次视觉 API，服务商调用失败也会占用一次。
...
每日用量只保存日期和次数，文件位于 `data/vision_usage.db`；图片、图片 URL、聊天正文和密钥不会写入该数据库。需要重新测试新的一天额度时等待本地日期变化，不要在机器人运行时修改数据库。
